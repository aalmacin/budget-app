...
CREATE OR REPLACE FUNCTION public._range_bounds(p_range TEXT)
...
ALTER FUNCTION public._range_bounds(TEXT) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION public._range_bounds(TEXT) FROM PUBLIC;
-- Internal helper.
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.cashflow_kpis(p_range TEXT)
RETURNS JSONB
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT start_date, end_date INTO v_start, v_end FROM public._range_bounds(p_range);
...
ALTER FUNCTION public.cashflow_kpis(TEXT) OWNER TO budget_function_owner;
...
-- essential_pct >= 50.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.essentials_breakdown(p_year INT, p_month INT)
RETURNS JSONB
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
ALTER FUNCTION public.essentials_breakdown(INT, INT) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION public.essentials_breakdown(INT, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.essentials_breakdown(INT, INT) TO authenticated;

-- ---------------------------------------------------------------------------
-- spend_over_time(p_range) — bucketed spend + income series.
-- 30d → daily buckets, 90d → weekly, ytd → monthly.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.spend_over_time(p_range TEXT)
RETURNS TABLE (
  bucket_start DATE,
  spent_cents  BIGINT,
  income_cents BIGINT
...
ALTER FUNCTION public.spend_over_time(TEXT) OWNER TO budget_function_owner;
