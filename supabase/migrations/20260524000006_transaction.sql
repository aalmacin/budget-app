-- T061 — Phase 7 / US6: budget.transaction, household-scoped.
--
...
CREATE INDEX transaction_household_occurred
  ON budget.transaction (household_id, occurred_on DESC);

CREATE INDEX transaction_household_category_occurred
  ON budget.transaction (household_id, category_id, occurred_on);
...
CREATE UNIQUE INDEX transaction_subscription_idempotency
  ON budget.transaction (subscription_id, occurrence_date)
  WHERE subscription_id IS NOT NULL AND occurrence_date IS NOT NULL;

CREATE INDEX transaction_notes_search
  ON budget.transaction USING GIN (to_tsvector('simple', notes));

CREATE TRIGGER transaction_set_updated_at
  BEFORE UPDATE ON budget.transaction
...
CREATE POLICY transaction_select ON budget.transaction
  FOR SELECT
  TO public
...
CREATE FUNCTION budget.enforce_member_household()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = ''
AS $$
...
CREATE CONSTRAINT TRIGGER transaction_member_household
  ...
  ON budget.transaction
  DEFERRABLE INITIALLY DEFERRED
  FOR EACH ROW EXECUTE FUNCTION budget.enforce_member_household();
