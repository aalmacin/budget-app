...
ALTER FUNCTION public.list_subscriptions() OWNER TO budget_function_owner;
...
ALTER FUNCTION public.pause_subscription(UUID) OWNER TO budget_function_owner;
...
ALTER FUNCTION public.resume_subscription(UUID) OWNER TO budget_function_owner;
...
BEGIN
  ...
  FROM norm n
  JOIN public.category c ON c.id = n.category_id
  GROUP BY c.name
  HAVING count(*) > 1
  ORDER BY c.name;
END;
...
ALTER FUNCTION public.list_overlapping_subscriptions() OWNER TO budget_function_owner;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.materialize_due_subscriptions(p_bypass_rls BOOLEAN DEFAULT false)
RETURNS INT
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  IF p_bypass_rls THEN
    ...
    v_household_id := NULL;
  ELSE
    v_household_id := public.get_current_household();
    IF v_household_id IS NULL THEN
      RETURN 0;
    END IF;
  END IF;
...
ALTER FUNCTION public.materialize_due_subscriptions(BOOLEAN) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION public.materialize_due_subscriptions(BOOLEAN) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.materialize_due_subscriptions(BOOLEAN) TO authenticated;
