...
ALTER FUNCTION public.get_current_household() OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION public.get_current_household() FROM PUBLIC;
...
AS $$
  ...
  FROM public.household_member hm
  WHERE hm.household_id = public.get_current_household()
    AND hm.deleted_at IS NULL
  ORDER BY hm.created_at
$$;

ALTER FUNCTION public.list_household_members() OWNER TO budget_function_owner;
...
--
-- The auth.users read is delegated to public.resolve_user_by_email() (see
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.add_adult_by_email(p_email TEXT)
...
BEGIN
  ...
  FROM public.resolve_user_by_email(p_email) r;
...
ALTER FUNCTION public.add_adult_by_email(TEXT) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION public.add_adult_by_email(TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.add_adult_by_email(TEXT) TO authenticated;

-- ---------------------------------------------------------------------------
-- add_kid(p_display_name, p_age_years) — inserts a kid in the caller's
-- household. No cap on kids.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.add_kid(p_display_name TEXT, p_age_years INT)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
ALTER FUNCTION public.add_kid(TEXT, INT) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION public.add_kid(TEXT, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.add_kid(TEXT, INT) TO authenticated;
...
ALTER FUNCTION public.soft_delete_member(UUID) OWNER TO budget_function_owner;
...
ALTER FUNCTION public.update_member_income(UUID, BIGINT) OWNER TO budget_function_owner;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.list_kid_month_summary(p_year INT, p_month INT)
RETURNS TABLE (
  kid_id            UUID,
  spent_cents       BIGINT,
...
ALTER FUNCTION public.list_kid_month_summary(INT, INT) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION public.list_kid_month_summary(INT, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.list_kid_month_summary(INT, INT) TO authenticated;
