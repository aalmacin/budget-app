...
BEGIN
  BEGIN
    ...
    INSERT INTO budget.household (name, owner_user_id) VALUES ('B', user_b)
      RETURNING id INTO household_b;

    INSERT INTO budget.household_member (household_id, user_id, role, display_name)
    ...
    SELECT id INTO v_seed_cat
      FROM budget.category WHERE household_id IS NULL AND name = 'Income';
    ...
    FOR i IN 1..5 LOOP
      ...
      BEGIN
        CASE v_table
          WHEN 'household' THEN
            INSERT INTO budget.household (name, owner_user_id) VALUES ('X', user_a);
          WHEN 'household_member' THEN
            INSERT INTO budget.household_member (household_id, role, display_name)
              VALUES (household_a, 'adult', 'X');
          WHEN 'category' THEN
            INSERT INTO budget.category (household_id, name) VALUES (household_a, 'X');
          WHEN 'transaction' THEN
            INSERT INTO budget.transaction (id, household_id, type, amount_cents, occurred_on, category_id)
              VALUES (gen_random_uuid(), household_a, 'expense', 1, current_date, v_seed_cat);
          WHEN 'subscription' THEN
            INSERT INTO budget.subscription (household_id, merchant, amount_cents, category_id, cadence, next_renewal_at)
              VALUES (household_a, 'X', 1, v_seed_cat, 'monthly', current_date);
        END CASE;
      EXCEPTION WHEN insufficient_privilege THEN blocked := TRUE;
      END;
      IF NOT blocked THEN
        RAISE EXCEPTION 'Lockdown test failed: authenticated could INSERT into budget.%', v_table;
      END IF;
    END LOOP;
    ...
    IF budget.get_current_household() <> household_a THEN
    ...
    IF budget.get_current_household() <> household_b THEN
