-- Fix: public.auth_user_household_ids() must be SECURITY DEFINER to avoid
-- infinite recursion through the household_member RLS policy.
--
-- The helper queries public.household_member. The RLS policy on
-- public.household_member is `household_id IN (SELECT * FROM
-- public.auth_user_household_ids())`. As SECURITY INVOKER under any non-
...
REVOKE ALL ON FUNCTION public.auth_user_household_ids() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.auth_user_household_ids() TO authenticated, anon;
