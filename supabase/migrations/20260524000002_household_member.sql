-- T057 — Phase 7 / US5: public.household_member + adult cap triggers.
-- Ported from legacy public.household_member (0002_household.sql), with the
-- timestamp trigger from the new shared helper.
...
CREATE FUNCTION public.enforce_adult_cap()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = ''
AS $$
...
CREATE FUNCTION public.forbid_undelete_of_adult_when_capped()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = ''
AS $$
