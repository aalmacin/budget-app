-- Fix two filter bugs in list_transactions that affect income rows.
--
-- Bug 1: Essential/Treats filter showed income rows because income defaults to
--   essential_pct=100 (satisfies >=50). Income is neither essential nor a treat,
--   so the filter now gates on t.type='expense'.
-- Bug 2: For-member filter missed income rows where the selected member is the
--   earner. The earner is stored in paid_by_member_id (not for_member_id) on
--   income rows, so the condition now also matches that field for income.
  ...
  WITH filtered AS (
    ...
    WHERE t.household_id = v_household_id
      ...
      AND (v_essential  IS NULL
           OR (v_essential = 'essential' AND t.type = 'expense' AND t.essential_pct >= 50)
           OR (v_essential = 'treats'    AND t.type = 'expense' AND t.essential_pct <  50))
      AND (v_for_member IS NULL
           OR t.for_member_id = v_for_member
           OR (t.type = 'income' AND t.paid_by_member_id = v_for_member))
