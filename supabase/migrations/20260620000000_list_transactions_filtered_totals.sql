-- Add filtered income/expense totals to list_transactions.
--
-- The transactions list is paginated, so summing the returned rows on the
-- client only covers the current page. These window aggregates run over the
-- full filtered set (before LIMIT/OFFSET) — same technique as total_count —
-- so the page can show totals that honour the active filters across all pages.
...
BEGIN
  ...
  SELECT f.id, f.type, f.amount_cents,
         ...
         coalesce(sum(f.amount_cents) FILTER (WHERE f.type = 'income')  OVER (), 0) AS total_income_cents,
         coalesce(sum(f.amount_cents) FILTER (WHERE f.type = 'expense') OVER (), 0) AS total_expense_cents
  FROM filtered f
