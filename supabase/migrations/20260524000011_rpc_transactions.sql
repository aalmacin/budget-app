...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION budget.list_transactions(p_filters JSONB)
RETURNS TABLE (
  id                      UUID,
  type                    TEXT,
  amount_cents            BIGINT,
...
BEGIN
  ...
  LEFT JOIN budget.category        c  ON c.id  = t.category_id
  LEFT JOIN budget.household_member fm ON fm.id = t.for_member_id
  LEFT JOIN budget.household_member pm ON pm.id = t.paid_by_member_id
...
ALTER FUNCTION budget.list_transactions(JSONB) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION budget.list_transactions(JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION budget.list_transactions(JSONB) TO authenticated;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION budget._insert_transaction(p_type TEXT, p JSONB)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
ALTER FUNCTION budget._insert_transaction(TEXT, JSONB) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION budget._insert_transaction(TEXT, JSONB) FROM PUBLIC;
-- Not granted to authenticated — internal helper only.

-- ---------------------------------------------------------------------------
-- log_expense(p jsonb) — wraps _insert_transaction('expense', ...).
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION budget.log_expense(p JSONB)
RETURNS UUID
LANGUAGE sql
SECURITY DEFINER
SET search_path = ''
AS $$
  SELECT budget._insert_transaction('expense', p)
$$;

ALTER FUNCTION budget.log_expense(JSONB) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION budget.log_expense(JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION budget.log_expense(JSONB) TO authenticated;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION budget.log_income(p JSONB)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  END IF;
  RETURN budget._insert_transaction('income', p);
END;
$$;

ALTER FUNCTION budget.log_income(JSONB) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION budget.log_income(JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION budget.log_income(JSONB) TO authenticated;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION budget.update_transaction(p_id UUID, p_patch JSONB)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
ALTER FUNCTION budget.update_transaction(UUID, JSONB) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION budget.update_transaction(UUID, JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION budget.update_transaction(UUID, JSONB) TO authenticated;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION budget.delete_transaction(p_id UUID)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
ALTER FUNCTION budget.delete_transaction(UUID) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION budget.delete_transaction(UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION budget.delete_transaction(UUID) TO authenticated;
...
-- a unified row shape.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION budget.list_quick_add_options(p_limit INT)
RETURNS TABLE (
  source         TEXT,
  ref_id         UUID,
  label          TEXT,
...
BEGIN
  ...
  subs AS (
    ...
    JOIN budget.category c ON c.id = s.category_id
...
ALTER FUNCTION budget.list_quick_add_options(INT) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION budget.list_quick_add_options(INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION budget.list_quick_add_options(INT) TO authenticated;
...
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION budget.get_dashboard_summary(p_year INT, p_month INT)
RETURNS JSONB
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
ALTER FUNCTION budget.get_dashboard_summary(INT, INT) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION budget.get_dashboard_summary(INT, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION budget.get_dashboard_summary(INT, INT) TO authenticated;
