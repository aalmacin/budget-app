...
CREATE OR REPLACE FUNCTION public._assert_household_category(p_category_id UUID)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  IF v_owner IS NULL THEN
    ...
    IF EXISTS (SELECT 1 FROM public.category WHERE id = p_category_id) THEN
...
ALTER FUNCTION public._assert_household_category(UUID) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION public._assert_household_category(UUID) FROM PUBLIC;
-- Internal helper; not granted to authenticated.

CREATE OR REPLACE FUNCTION public.set_category_essential_pct(p_category_id UUID, p_pct INT)
RETURNS VOID
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  END IF;
  PERFORM public._assert_household_category(p_category_id);
...
ALTER FUNCTION public.set_category_essential_pct(UUID, INT) OWNER TO budget_function_owner;
REVOKE ALL ON FUNCTION public.set_category_essential_pct(UUID, INT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.set_category_essential_pct(UUID, INT) TO authenticated;
...
ALTER FUNCTION public.set_category_budget(UUID, BIGINT) OWNER TO budget_function_owner;
...
ALTER FUNCTION public.get_budget_progress(INT, INT, TEXT) OWNER TO budget_function_owner;
