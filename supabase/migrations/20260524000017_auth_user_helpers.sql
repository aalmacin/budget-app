...
--
-- Why this exists: `budget.create_household` (T064) and `budget.add_adult_by_email`
...
CREATE OR REPLACE FUNCTION budget.current_user_display_name()
RETURNS TEXT
LANGUAGE sql
SECURITY DEFINER
SET search_path = ''
...
REVOKE ALL ON FUNCTION budget.current_user_display_name() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION budget.current_user_display_name() TO budget_function_owner;


CREATE OR REPLACE FUNCTION budget.resolve_user_by_email(p_email TEXT)
...
REVOKE ALL ON FUNCTION budget.resolve_user_by_email(TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION budget.resolve_user_by_email(TEXT) TO budget_function_owner;
...
CREATE OR REPLACE FUNCTION budget.bootstrap_household(
  p_user_id      UUID,
  p_name         TEXT,
  p_display_name TEXT
)
...
REVOKE ALL ON FUNCTION budget.bootstrap_household(UUID, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION budget.bootstrap_household(UUID, TEXT, TEXT) TO budget_function_owner;
