...
8. **Income**: drawer → Add Income → enter $5,800 **net** for Alex (source `Salary`); $2,485 net for Bea (source `Salary`). The income-split rule view in Settings now shows ~70/30 derived ratios. No tax-bucket aside is created — v1 income is entered as net (clarification §6); friendlier source labels per clarification §9.
