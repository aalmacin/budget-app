...
- [ ] T057 [US2] Dashboard page (calls `rpc('get_dashboard_summary')`) with sage hero "Left to spend · <month>", essential/treats split card, recent-activity strip, FAB linking to `/quick-add` in `app/(app)/dashboard/page.tsx`
...
- [ ] T078 [P] [US4] Family screen (member list + add adult by email + add kid + edit income; hides soft-deleted; per-kid cards show **spent-this-month only — NO per-kid budget bar** per spec clarification §9) in `app/(app)/family/page.tsx`
