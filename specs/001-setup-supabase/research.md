...
**Cross-schema read addendum (2026-05-25)**: Two RPCs — `create_household` and `add_adult_by_email` — need to read `auth.users` (for `email` and `raw_user_meta_data->>'full_name'`). The `budget_function_owner` role inherits from `authenticated`, which has no SELECT on `auth.users`, so the direct reads in the original implementation failed at runtime with `permission denied for table users` (reported 2026-05-24 from `/onboarding/create-household`). Resolution: migration `20260524000017_auth_user_helpers.sql` introduces two `postgres`-owned `SECURITY DEFINER` helpers — `budget.current_user_display_name()` and `budget.resolve_user_by_email(text)` — that expose only the column-scoped reads the callers need. `EXECUTE` on each is granted to `budget_function_owner`. The function-owner role itself remains without any direct grant on `auth.users`, so PII exposure stays minimal (Principle II). This pattern is the one prescribed by FR-035 for any future RPC that needs to read outside the `budget` schema.

**RLS chicken-and-egg addendum (2026-05-25, follow-up)**: While verifying the C1 fix above, the household RLS test surfaced a second pre-existing bug: `create_household` insert-then-insert (household + first member) fails the household RLS WITH CHECK because the caller has zero memberships at the moment of the first INSERT. An OR-carve-out in WITH CHECK (`id ∈ memberships OR owner_user_id = auth.uid()`) was attempted and reverted — the combined expression evaluated as NULL on the local Postgres, traceable to STABLE-function caching of `auth.uid()` across the SECURITY DEFINER helper subquery boundary; the planner treats a NULL WITH CHECK as a policy violation. `SET LOCAL row_security = off` is not available to `budget_function_owner` (which lacks BYPASSRLS). Resolution: the same `20260524000017_auth_user_helpers.sql` migration adds a third `postgres`-owned helper, `budget.bootstrap_household(p_user_id, p_name, p_display_name)`, which performs the two atomic INSERTs while bypassing RLS via its superuser ownership, and verifies the caller's `auth.uid()` matches the bootstrapped owner. `create_household` is now a thin wrapper: validate inputs, derive display name, delegate to `bootstrap_household`. The household and household_member policies stay clean (single membership-based clause). FR-022a documents this contract; FR-035 documents the broader "postgres-owned helper" pattern.
