...
- **G2** — `SAVEPOINT rls_test` now opens BEFORE the synthetic `auth.users` inserts, so `ROLLBACK TO SAVEPOINT` also rolls them back. The RLS test migration now leaves no residue across `db reset` runs.
