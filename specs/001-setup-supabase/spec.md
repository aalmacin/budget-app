...
- **FR-022a**: The first-adult member's `display_name` MUST be derived from `auth.users` for the calling user, preferring `raw_user_meta_data->>'full_name'` over the email local-part, with `'Adult'` as the final fallback. The `auth.users` read MUST go through a `postgres`-owned `SECURITY DEFINER` helper (see FR-035) so the function-owner role is not granted broad SELECT on `auth.users`. The two atomic INSERTs (household + first member) MUST also go through a `postgres`-owned `SECURITY DEFINER` bootstrap helper because the caller has zero memberships at that moment and the household/household_member RLS WITH CHECK ("household_id ∈ caller's memberships") therefore fails for the function-owner role; the bootstrap helper bypasses RLS for that single two-row operation and verifies the caller's `auth.uid()` matches the bootstrapped owner.
...
- **FR-035**: Every Phase 7 RPC MUST be defined with `SECURITY DEFINER`, `LANGUAGE plpgsql` (or `sql` where pure), `SET search_path = ''` (every table reference schema-qualified), `OWNER TO budget_function_owner`, and `GRANT EXECUTE TO authenticated`. Functions on the cron path that need to operate across households MAY use `SET LOCAL row_security = off` only in a clearly demarcated branch of the function body; the function comment MUST document this. Any RPC that needs to read a table OUTSIDE the `budget` schema (notably `auth.users`) MUST NOT do so directly; instead it MUST call a narrowly-scoped `SECURITY DEFINER` helper owned by `postgres` (which has the necessary privileges by default) and grant `EXECUTE` on that helper to `budget_function_owner`. This keeps PII exposure column-scoped and avoids handing the function-owner role broad cross-schema SELECT.
