...
A user with an existing account opens the Budget app, is prompted to sign in with email and password, and on success lands on the app's main page where their budget workspace will live. Without a valid session, every non-login page redirects them back to the sign-in screen.
...
**Independent Test**: With a pre-provisioned account, visit any app URL, get redirected to the sign-in page, submit valid credentials, and confirm you land on the main page. Visiting the app again in a new tab keeps the session active without re-entering credentials.

**Acceptance Scenarios**:

1. **Given** a visitor with no active session, **When** they visit any in-app URL, **Then** they are redirected to the sign-in page.
2. **Given** the sign-in page is shown, **When** the user submits valid email and password, **Then** they are redirected to the main page and remain signed in across navigation and reloads.
...
- **FR-013**: Any tables, views, or functions created for this app MUST live under this project's dedicated database schema (`budget`) on the shared database instance, isolated from tables belonging to other apps on the same instance.
...
- **SC-001**: A user with valid credentials can move from the sign-in page to the main page in under 5 seconds on a normal broadband connection.
...
- The Budget app uses a shared database instance that hosts other applications as well, and this app's tables, policies, and migrations are confined to its own dedicated schema (`budget`) to avoid collisions. This is recorded as a project-level constraint.
